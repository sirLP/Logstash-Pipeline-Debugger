            ...
            for pkt in packets:
                if pkt.haslayer(Raw):
                    ...
                    try:
                        ...
                        # Skip empty or binary-looking payloads
                        if not payload_str or len(payload_str) < 10:
                            continue
                        
                        # Check if it looks like a syslog message
                        if '<' in payload_str[:10] or any(keyword in payload_str[:100] for keyword in ['filterlog', 'dhcpd', 'unbound', 'suricata']):
                            log_entry = {
                                'payload': payload_str,
                                'timestamp': pkt.time if hasattr(pkt, 'time') else None,
                                'src_ip': pkt[1].src if len(pkt.layers()) > 1 and hasattr(pkt[1], 'src') else None,
                                'dst_ip': pkt[1].dst if len(pkt.layers()) > 1 and hasattr(pkt[1], 'dst') else None,
                            }
